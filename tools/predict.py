...
if __name__ == "__main__":
    ...
    if dataset in ['can','walnuts']:
        ...
        cmd = f"python test.py --data_path {data_path} --save_dir {save_dir} --gpu {gpu} --item {dataset}"
        print(f"command: {cmd}")
        os.system(cmd)
    else:
        os.chdir("./glass2")
        ...
        cmd = f"python predict.py --results_path {save_dir} --gpu {gpu} --seed 0 --test ckpt net -b auto --pretrain_embed_dimension 512 --target_embed_dimension 512 --patchsize 3 --meta_epochs 640  --dsc_layers 2 --dsc_hidden 1024 --pre_proj 1 --mining 1 --noise 0.015 --radius 0.75 --p 0.5 --step 20 --limit 392 "\
