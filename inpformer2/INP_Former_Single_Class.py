...
def main(args):
    ...
    if args.dataset == 'MVTec-AD2':
        train_data_transform, test_data_transform, gt_transform = get_strong_transforms(args.input_size, args.crop_size)
        train_path = os.path.join(args.data_path, args.item, 'train')
        val_path = os.path.join(args.data_path, args.item, 'validation')
        test_path = os.path.join(args.data_path, args.item)

        train_data = ImageFolder(root=train_path, transform=train_data_transform)
        val_data = ImageFolder(root=val_path, transform=train_data_transform)
        combined_train_data = ConcatDataset([train_data, val_data])
        test_data = MVTec2Dataset(root=test_path, transform=test_data_transform, gt_transform=gt_transform, phase="test_public")
        train_dataloader = torch.utils.data.DataLoader(combined_train_data, batch_size=args.batch_size, shuffle=True, num_workers=4,
                                                       drop_last=True)
        test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size, shuffle=False, num_workers=4)
    elif args.dataset == 'MVTec-AD' or args.dataset == 'VisA':
        train_path = os.path.join(args.data_path, args.item, 'train')
        test_path = os.path.join(args.data_path, args.item)
