...
def get_strong_transforms_sig(size, isize, mean_train=None, std_train=None):
    ...
    list_aug = [
        ...
        # transforms.GaussianBlur(kernel_size=15, sigma=4),
    ]
    ...
    #     transforms.RandomResizedCrop((isize, isize), scale=(0.8, 1.1)),
    ...
    test_data_transforms = transforms.Compose([
        transforms.Resize((isize, isize)),
        transforms.ToTensor(),
        # transforms.CenterCrop(isize), # (h,w)
        # transforms.Normalize(mean=mean_train,
        #                      std=std_train)
        ])
...
def get_strong_transforms_without_norm(size, isize, mean_train=None, std_train=None):
    # mean_train = [0.485, 0.456, 0.406] if mean_train is None else mean_train
    # std_train = [0.229, 0.224, 0.225] if std_train is None else std_train
    train_data_transforms = seg_transforms.Compose([
        seg_transforms.Resize((size, size)),
        seg_transforms.RandomResizedCrop((isize, isize), scale=(0.8, 1.1)),
        seg_transforms.RandomHorizontalFlip(),
        seg_transforms.RandomVerticalFlip(),
        transforms.RandomChoice([
            seg_transforms.ColorJitter(brightness=0.2),
            seg_transforms.ColorJitter(contrast=0.2),
            seg_transforms.ColorJitter(saturation=0.2),
            seg_transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1)
            ]),
        seg_transforms.ToTensor(),
        # seg_transforms.Normalize(mean=mean_train,
        #                      std=std_train)
        ])

    train_data_transforms2 = seg_transforms.Compose([
        seg_transforms.Resize((size, size)),
        seg_transforms.RandomResizedCrop((isize, isize), scale=(0.8, 1.1)),
        seg_transforms.RandomHorizontalFlip(),
        # seg_transforms.RandomVerticalFlip(),
    ...
    test_data_transforms = seg_transforms.Compose([
        seg_transforms.Resize((isize, isize)),
        seg_transforms.ToTensor(),
        # seg_transforms.Normalize(mean=mean_train,
        #                      std=std_train)
        ])
    return train_data_transforms, train_data_transforms2, test_data_transforms
...
# 只载入good图像，good图像的gt随图片一起做增强
class MVTec2DatasetV2(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.phase != 'test':
            if label == 0:
                ...
                gt = Image.new(mode='L', size=img.size, color=0)
            else:
                gt = Image.open(gt)
            ...
            img, gt = self.transform((img, None))
            label = None
        
        return img, gt, label, img_path

# 只载入good图像，good图像的gt直接生成
...
# 载入所有图像，good图像的gt随图片一起做增强
class MVTec2DatasetV3(torch.utils.data.Dataset):
    def __init__(self, root, transform, phase):
        assert phase in ('train', 'test', 'val')
        self.data_path = []
        self.phase = phase
