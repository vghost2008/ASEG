...
from dataset2 import get_strong_transforms, MVTec2DatasetV2
...
def main(args):
    ...
    if args.dataset == 'MVTec-AD2':
        train_data_transform, test_data_transform = get_strong_transforms(args.input_size, args.crop_size)
        data_root = os.path.join(args.data_path, args.item)
        train_data = MVTec2DatasetV2(root=data_root, transform=train_data_transform, phase='train')
        val_data = MVTec2DatasetV2(root=data_root, transform=test_data_transform, phase='val')
    ...
    if args.phase == 'train':
        ...
        lr_scheduler = WarmCosineScheduler(optimizer, base_value=1e-3, final_value=1e-4, total_iters=args.total_epochs*len(train_dataloader),
                                           warmup_iters=100)
        print_fn(f'{args.item} train image number:{len(train_data)}')
        ...
        for epoch in range(args.total_epochs):
            ...
            for data in tqdm(train_dataloader, ncols=80):
                ...
                lr_scheduler.step()
            print_fn('train {} epoch [{}/{}], loss:{:.4f}'.format(args.item, epoch+1, args.total_epochs, np.mean(loss_list)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_name', type=str, default='INP-Former-Single-Class-Pro')
